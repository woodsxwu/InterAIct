            ...
            with col:
                with st.container():
                    ...
                    if st.button(f"Play this scenario", key=f"scenario_{scenario['id']}", use_container_width=True):
                        # Set the current scenario index based on this selection
                        st.session_state.current_scenario_index = i
                        # Reset current phase to start at the beginning of the scenario
